...
var quiLanguage={
	dialog:{
		...
		minButtonTitle:"最小化",
		closeButtonTitle:"关闭",
		decreaseButtonTitle:"还原",
		autoCloseMessage:"秒后自动关闭",
		errorMessage:"没有取到对应ID的弹出框页面对象",
		errorMessage2:"可能存在访问限制，不能获取到浮动窗口中的文档对象。",
		alertTitleMessage:"系统提示",
		confirmTitleMessage:"信息确认"
	}
};
...
Dialog.prototype.create = function () {
    ...
    if(this.Style=="normal"){
         var Height=this.Height>110?"height:"+this.Height + "px;":"min-height: " +this.Height + "px;";
        html = '\
  <table id="_DialogTable_{thisID}" width="' + (this.Width + this.dialogWidthFix) + '" cellspacing="0" cellpadding="0" border="0" style="font-size:' + (this.defaultFontSize) + 'px; font-family:' + (this.defaultFontFamily) + '; line-height:1.4;border-collapse: collapse;">\
    ...
    <tr valign="top">\
      ...
      <td align="center"><table width="100%" cellspacing="0" cellpadding="0" border="0" bgcolor="#ffffff">\
          ...
          <tr>\
            <td valign="top"><div id="_Container_{thisID}" style="text-align:left;position: relative;width: ' + + (this.Width + 26) + 'px; '+Height+'">\
    ...
    ' + (function (obj) {
        ...
        return "";
          ...
          <tr id="_ButtonRow_{thisID}" style="' + (this.ShowButtonRow ? "" : "display:none") + '">\
            <td height="36"><div id="_DialogButtons_{thisID}" style="text-align:' + this.ButtonAlign + '; border-top: 1px solid #DADEE5; padding: 8px 20px;">\
                <input type="button" class="btn btn-primary" style="' + (this.ShowOkButton ? "" : "display:none") + '"  value="' + this.OkButtonText + '" id="_ButtonOK_{thisID}"/>\
                <input type="button" class="btn default" style="' + (this.ShowCancelButton ? "" : "display:none") + '"  value="' + this.CancelButtonText + '" onclick="Dialog.getInstance(\'{thisID}\').close();" id="_ButtonCancel_{thisID}"/>\
              </div></td>\
          </tr>\
        </table></td>\
...
';
    ...
    else if(this.Style=="custom"){//自定义风格
        html ='\
  <table id="_DialogTable_{thisID}" width="' + (this.Width + this.dialogWidthFix) + '" cellspacing="0" cellpadding="0" border="0" style="font-size:' + (this.defaultFontSize) + 'px; font-family:' + (this.defaultFontFamily) + ';line-height:1.4;border-collapse: collapse;">\
    <tr id="_Draghandle_{thisID}" style="display:none;" onselectstart="return false;" style="-moz-user-select: -moz-none; ' + (this.Drag ? "cursor: move;" : "") + '">\
          ...
          <tr>\
            <td valign="top"><div id="_Container_{thisID}" style="text-align:left;position: relative; width: ' + + (this.Width + 26) + 'px; height: ' + this.Height + 'px;"">\
                <div  style="position: absolute; height: 100%; width: 100%; display: none; background-color:#fff; opacity: 0.5;" id="_Covering_{thisID}">&nbsp;</div>\
    ' + (function (obj) {
        if (obj.InnerHtml) return obj.InnerHtml;
        ...
        return "";
          ...
          <tr id="_ButtonRow_{thisID}" style="display:none;" >\
            <td height="36"><div id="_DialogButtons_{thisID}" style="border-top: 1px solid #DADEE5; padding: 8px 20px; text-align: right; ">\
                <input type="button" class="btn btn-primary" style="' + (this.ShowOkButton ? "" : "display:none") + '"  value="' + this.OkButtonText + '" id="_ButtonOK_{thisID}"/>\
                <input type="button" class="btn default" style="' + (this.ShowCancelButton ? "" : "display:none") + '"  value="' + this.CancelButtonText + '" onclick="Dialog.getInstance(\'{thisID}\').close();" id="_ButtonCancel_{thisID}"/>\
              </div></td>\
          </tr>\
...
Dialog.prototype.max = function () {
  ...
  //如果已经最大化，则还原
   if(this.maxFlag==true){
        ...
        topWin.$id("_Container_" + this.ID).style.width = this.Width + "px";
        topWin.$id("_Container_" + this.ID).style.height = this.Height + "px";
        $("#_Draghandle_"+ this.ID).find(".dialog_maxbtn").find('i').removeClass("fa-compress").addClass('fa-arrows-alt');
    ...
    //最大化
        ...
        $("#_Draghandle_"+ this.ID).find(".dialog_maxbtn").find('i').removeClass("fa-arrows-alt").addClass('fa-compress');
...
Dialog.prototype.min = function () {
   ...
   if(tr[0].style.display=="none"){
     ...
     topWin.$id("_Container_" + this.ID).style.width = this.Width + "px";
     topWin.$id("_Container_" + this.ID).style.height = this.Height + "px";
     $("#_Draghandle_"+ this.ID).find(".dialog_minbtn").find('i').removeClass('fa-expand').addClass('fa-minus');
    $("#_Draghandle_"+ this.ID).find(".dialog_minbtn").attr("title",this.minButtonTitle);
     tr.show();
     if(this.DecreaseResetPosition){
         this.setPosition();
   ...
   //最小化
    ...
    // $("#_Draghandle_"+ this.ID).find(".dialog_minbtn").remove();//mark
     $("#_Draghandle_"+ this.ID).find(".dialog_minbtn").find('i').removeClass('fa-minus').addClass('fa-expand')
...
Dialog.prototype.getBgdiv = function () {
   ...
   var opactty2=String(this.Alpha/100);
    ...
    if (!bgdiv) {
        ...
        var bgIframe;
        if(this.IframeBg){
            bgIframe = '<iframe src="about:blank" style="filter:alpha(opacity=0);" width="100%" height="100%"></iframe>';
        }
        else{
            bgIframe='';
        }
        ...
        if (this.IframeBg) {
            ...
            bgIframeDoc.open();
            bgIframeDoc.write("<body oncontextmenu='return false;'></body>");
            bgIframeDoc.close();
            bgIframeDoc=null;
        }
    }
...
Dialog.alert = function (msg, func, w, h,autoClose) {
    ...
    if(msg){
        ...
        if(msgArray.length>1){
         ...
         diag.InnerHtml = '<div>\
            <div><div class="dialog_td">'
            +'<div class="sweet-alert sweet-alert-content"   style="display: block;">'
            ...
            +'</div>'
            +'</div>\
                <div class="dialog_td" id="Message_' + this.ID + '" style="text-align:center;font-size:' + (diag.defaultFontSize) + 'px;font-family:' + (diag.defaultFontFamily) + '">' + msgArray[0] + '</div></div>\
        </div>';
    }
   else{
        diag.Title = diag.alertTitleMessage;
    ...
    if(diag.cancelButton){
         $(diag.cancelButton).removeClass('btn default').addClass('btn btn-primary');
...
Dialog.confirm = function (msg, funcOK, funcCal, w, h) {
    ...
    diag.InnerHtml = '<div>\
        <div><div class="dialog_td"><div class="sweet-alert sweet-alert-content"  style="display: block;">'
...
Dialog.error = function (msg, func, w, h,autoClose) {
    var w = w || 300,
        h = h || 110;
    var diag = new Dialog({
        Width: w,
...
Dialog.warning = function (msg, func, w, h,autoClose) {
    var w = w || 300,
        h = h || 110;
    var diag = new Dialog({
        Width: w,
    ...
    if(msg){
        ...
        <div><div class="dialog_td"><div class="sweet-alert sweet-alert-content"  style="display: block;">'
                           ...
                           +'<span class="sa-dot pulseWarningInst"></span>'
                           +'<div class="sa-placeholder"></div>'
                          +'<div class="sa-fix"></div>'
                          +'</div>'
                          +'</div></div>'

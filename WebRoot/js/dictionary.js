...
function dictModule_initNormalPopTree() {
	$(".dict_normalPopTree").click(function(){
		...
		var selectedWin = window.open("dictTree/showDictTree?muti="+muti+"&ajax="+ajax+"&dynroot="+dynroot+"&expand="+expand+"&context="+context+"&dictId=" + dictId,"selectedWin", "width=400,height=500,scrollbars=0,status=1","location=no");
		selectedWin.focus();
	});
	//Tree icon
...
function dictModule_initLargeAIDictSelecter() {
	...
	$(".dict_largeAIImg").click(function(){
		...
		var dictId=$(this).attr("dictId");
		var selectedWin = window.open("dictTree/showDictTree?muti=false&ajax=true&dictId=" + dictId,"selectedWin", "width=400,height=500,scrollbars=0,status=1","location=no");
		selectedWin.focus();
	});
	//延时加载iframe
	this.loadIFrame=function(){
...
function dictModule_initReignDict(){
	...
	reigns.change(function(){
		...
		if(value!=""&&reign!=""){
			...
			$.get("dictTree/schTreeSub?dictId="+dictId+"&id="+value,function(data){
				if(data.length >0){

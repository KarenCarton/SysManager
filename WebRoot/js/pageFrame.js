...
function init_page(){
	...
	*********************************************/
	...
	/********************************************
	   更改滚动条样式（页面需引入jquery.nicescroll.min.js）
 	*********************************************/
	if($(".nicescroll").length>0){
		$(".nicescroll").niceScroll({
			cursorcolor:"#2ec8ca",//设置光标颜色
			cursorwidth:'2px',//设置光标宽度
			autohidemode:false,//设置光标非活动时是否隐藏
			cursorborder:'1px solid rgb(177, 177, 177)',//设置光标边框
			cursorborderradius:'10px',//设置光标圆角
			background:'rgba(237,237,237,0.15)',//设置轨道背景色
			railpadding:{top:1,right:1,bottom:1,left:1},//设置滚动条距离边框的距离
			cursorminheight:40,//设置光标的高度
		});
	}
	//横轴隐藏滚动
	if($(".nicescrollH").length>0){
		$(".nicescrollH").niceScroll({
			cursorcolor:"#2ec8ca",//设置光标颜色
			cursorwidth:'2px',//设置光标宽度
			autohidemode:false,//设置光标非活动时是否隐藏
			cursorborder:'1px solid rgb(177, 177, 177)',//设置光标边框
...
function preventDoubleClick(seconds){
   if(isClick){
      ...
      isClick=true;
      setTimeout(()=>{isClick=false},seconds||500)
      return false
   }
}
